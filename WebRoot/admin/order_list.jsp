  ...
  <tbody>
    ...
    <!--  <td><label><input type="checkbox" class="ace"><span class="lbl"></span></label></td> -->
     ...
     <c:if test="${o.status=='成功订单' }">
     	<a title="退房"  href="tuiFang.do?id=${o.id }" class="btn btn-xs btn-status" >退房</a> 
     </c:if>
     <c:if test="${o.status=='已住' }">
     	<a title="退房"  href="tuiFang.do?id=${o.id }" class="btn btn-xs btn-status" >退房</a> 
     </c:if>
     <c:if test="${o.status=='预约,待确认' }">
     	<a title="接收订单"  href="order_admin_tongYi.do?id=${o.id }" class="btn btn-xs btn-status" >接收订单</a> 
     	<a title="不接收"  href="order_admin_buTongYi.do?id=${o.id }" class="btn btn-xs btn-status" >不接收</a> 
     </c:if>
     <c:if test="${o.status=='已支付，待确认' }">
     	<a title="出房"  href="order_admin_chuFang.do?id=${o.id }" class="btn btn-xs btn-status" >确认</a> 
     </c:if>
    
     <c:if test="${o.status=='待确认退房' }">
     	<a title="退房"  href="tuiFang.do?id=${o.id }"  class="btn btn-xs btn-status" >退房</a> 
     </c:if>
    ...
    <td colspan="20">
               当前第${page.page}页；共${page.totalPage}页；总共${page.total}条记录；
				<c:if test="${page.page>1}">
              	<a href="orderList.do?page=1" >首页</a>
             	<a href="orderList.do?page=${page.page-1 }"> 上一页</a> 
             	</c:if>
    	     	<c:if test="${page.page<page.totalPage}">
				<a href="orderList.do?page=${page.page+1 }">下一页</a>
				<a href="orderList.do?page=${page.totalPage }">末页</a>
		    	</c:if> 
    ...
    <td colspan="20">
				...
				<c:if test="${page.page>1}">
              	<a href="selectForderList.do?page=1" >首页</a>
             	<a href="selectForderList.do?page=${page.page-1 }"> 上一页</a> 
             	</c:if>
    	     	<c:if test="${page.page<page.totalPage}">
				<a href="selectForderList.do?page=${page.page+1 }">下一页</a>
				<a href="selectForderList.do?page=${page.totalPage }">末页</a>
		    	</c:if> 
