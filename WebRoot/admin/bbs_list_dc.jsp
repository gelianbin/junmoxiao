...
<form method="post" action="vaguebbsList_dc.do">
 ...
 <table class="table table_list table_striped table-bordered" id="">
  ...
  <c:forEach items="${list }" var="h" varStatus="num">
     <tr>
        ...
        <td>${h.status }</td>
        <td class="td-manage">
        <c:if test="${h.status=='待处理' }">
         <a  href="doUpdateBbs_dc.do?id=${h.id }"  class="btn btn-xs btn-info" >处理</a> 
        </c:if>
     ...
     <c:if test="${sessionScope.p==1 }">
		<td colspan="12">当前第${page.page}页；共${page.totalPage}页；总共${page.total}条记录； <c:if test="${page.page>1}">
              <a href="bbsList_dc.do?page=1" >首页</a>
             <a href="bbsList_dc.do?page=${page.page-1 }"> 上一页</a> 
             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="bbsList_dc.do?page=${page.page+1 }">下一页</a>
			<a href="bbsList_dc.do?page=${page.totalPage }">末页</a>
		    </c:if> 
		</td>
	</c:if>
